...
export function redactEmail(email: string): string {
    ...
    if (dotIndex === -1) return `${redactedLocal}@***`;
 ...
 */
export function redactPII(obj: unknown, depth: number = 0): unknown {
...
export function redactError(error: unknown): Record<string, unknown> {
    if (error instanceof Error) {
        return {
            ...
            ...(redactPII((error as any).details || {}) as Record<string, unknown>)
        };
    }

    if (typeof error === 'object' && error !== null) {
        return redactPII(error) as Record<string, unknown>;
    }
