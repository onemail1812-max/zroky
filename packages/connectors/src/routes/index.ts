...
export function createRoutes(redis: Redis): Router {
    const router = Router();
    ...
    const requireTenantContext = (req: Request, res: Response, next: NextFunction) => {
        // Skip for callbacks (context is in state param)
    ...
    router.post('/connect/google', async (req, res) => {
        ...
        try {
            ...
            authUrl.searchParams.set('redirect_uri', `${process.env.API_BASE_URL}/api/v1/connectors/oauth/callback/google`);
    ...
    router.post('/connect/microsoft', async (req, res) => {
        ...
        try {
            ...
            authUrl.searchParams.set('redirect_uri', `${process.env.API_BASE_URL}/api/v1/connectors/oauth/callback/microsoft`);
            authUrl.searchParams.set('response_type', 'code');
            authUrl.searchParams.set('scope', requestedScopes.join(' '));
    ...
    router.get('/oauth/callback/google', async (req, res) => {
        console.error('DEBUG: Google Callback Entered');
        console.error('SECRET LEN:', process.env.GOOGLE_CLIENT_SECRET?.length);
        console.error('SECRET START:', process.env.GOOGLE_CLIENT_SECRET?.substring(0, 5));
        ...
        try {
            ...
            const tokenRequestBody = new URLSearchParams({
                ...
                client_secret: process.env.GOOGLE_CLIENT_SECRET!,
                code: code as string,
                redirect_uri: `${process.env.API_BASE_URL}/api/v1/connectors/oauth/callback/google`,
            ...
            if (!tokenResponse.ok) {
                ...
                logger.error({ errorText, status: tokenResponse.status }, 'Google Token Exchange Failed');
            ...
            if (!userInfo.id && !userInfo.sub) {
                logger.error({ userInfo }, 'Google UserInfo missing ID/sub');
                throw new Error('Google UserInfo missing ID/sub');
            }

            if (!userInfo.email) {
                logger.error({ userInfo }, 'Google UserInfo missing email');
            ...
            const account = await prisma.connectedAccount.upsert({
                ...
                create: {
                    ...
                    status: 'ACTIVE',
                    enabledScopes: JSON.stringify(tokens.scope.split(' ')),
                    requestedScopes: JSON.stringify(tokens.scope.split(' '))
                },
                update: {
                    status: 'ACTIVE',
                    enabledScopes: JSON.stringify(tokens.scope.split(' '))
        ...
        try {
            ...
            const tokenResponse = await fetch('https://login.microsoftonline.com/common/oauth2/v2.0/token', {
                ...
                body: new URLSearchParams({
                    ...
                    code: code as string,
                    redirect_uri: `${process.env.API_BASE_URL}/api/v1/connectors/oauth/callback/microsoft`,
                    grant_type: 'authorization_code'
                })
            });
    ...
    router.get('/accounts', async (req, res) => {
        ...
        try {
            ...
            const parsedAccounts = accounts.map(account => ({
                ...account,
                enabledScopes: account.enabledScopes ? JSON.parse(account.enabledScopes) : []
            }));

            res.json({ accounts: parsedAccounts });
        } catch (error) {
    ...
    router.get('/accounts/:accountId/email/threads', validateAccount, async (req, res) => {
        ...
        try {
            ...
            const result = await adapter.listThreads(account.id, {
                maxResults: parseInt(req.query.limit as string) || 50,
    ...
    router.post('/accounts/:accountId/email/drafts/:draftId/send', validateAccount, async (req, res) => {
        ...
        try {
            ...
            // Validate approval
            const validation = await approvalService.validateApproval(
                approvalId,
                tenantId,
                userId,
                'SEND_EMAIL'
            ...
            await timer.failure(
                error instanceof ConnectorApiError ? error.code : 'UNKNOWN_ERROR'
            );
            handleApiError(res, error, correlationId);
        }
    });
    ...
    router.get('/accounts/:accountId/calendar/events', validateAccount, async (req, res) => {
        ...
        try {
            ...
            const result = await adapter.listEvents(account.id, {
                ...
                timeMin: req.query.timeMin ? new Date(req.query.timeMin as string) : undefined,
                timeMax: req.query.timeMax ? new Date(req.query.timeMax as string) : undefined,
                maxResults: parseInt(req.query.limit as string) || 50,
                pageToken: req.query.pageToken as string,
...
function handleApiError(res: Response, error: unknown, correlationId: string) {
