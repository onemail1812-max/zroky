...
export class GoogleGmailAdapter {
    ...
    async trashMessages(accountId, messageIds) {
        const ctx = await this.getClient(accountId);
        try {
            await Promise.all(messageIds.map(id => ctx.gmail.users.messages.trash({ userId: 'me', id })));
        }
        catch (error) {
            throw this.normalizeError(error);
        }
    ...
    async getIncrementalChanges(accountId, options) {
        ...
        try {
            if (!options.historyId) {
                throw new ConnectorApiError('MISSING_HISTORY_ID', 'historyId is required for incremental sync', 400);
            }
    ...
    async setupWatch(accountId, options) {
        ...
        try {
            ...
            if (!topicName) {
                throw new ConnectorApiError('CONFIG_ERROR', 'GOOGLE_PUBSUB_TOPIC environment variable is required', 500);
            }
    ...
    normalizeThread(thread, accountId, includeBody) {
        ...
        for (const msg of messages) {
            ...
            if (from)
                participantSet.set(from.email, from);
    ...
    normalizeMessage(message, accountId, includeBody) {
    ...
    normalizeDraft(draft, accountId) {
    ...
    normalizeLabel(label, accountId) {
        return {
            id: label.id,
            accountId,
            name: label.name,
    ...
    normalizeError(error) {
        ...
        if (err.code && err.message) {
            ...
            if (statusCode === 429) {
                return new ConnectorApiError('RATE_LIMITED', 'Too many requests to Gmail API', 429, true, { retryAfter: 60 });
            }
            // Check for auth errors
            if (statusCode === 401 || statusCode === 403) {
                return new ConnectorApiError('AUTH_ERROR', err.message, statusCode, false);
            }
            return new ConnectorApiError('PROVIDER_ERROR', err.message, statusCode, statusCode >= 500);
        }
