...
async def search(
    ...
    try:
        ...
        logger.error(f"Search failed for workspace {workspace_id}: {e}", exc_info=True)
        raise HTTPException(
...
@cached_response(ttl_seconds=60, prefix="assist_thread")
async def get_thread(
    ...
    try:
        ...
        logger.error(f"Failed to fetch thread {thread_id} for workspace {workspace_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Unable to retrieve thread details"
        )

@router.get("/event/{event_id}")
@cached_response(ttl_seconds=60, prefix="assist_event")
async def get_event(
    request: Request,
    event_id: str,
    provider: str = "google",
    ...
    try:
        ...
        logger.error(f"Failed to fetch event {event_id} for workspace {workspace_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Unable to retrieve event details"
        )

@router.get("/messages")
@cached_response(ttl_seconds=30, prefix="assist_messages")
async def get_messages(
    ...
    try:
        ...
        logger.error(f"Failed to list messages for workspace {context.workspace_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def answer(
    ...
    orchestrator = AaliyahOrchestrator.get_orchestrator(workspace_id, brain)
    # Intent and context handling is encapsulated within orchestrator.handle_chat
    ...
    try:
        ...
        logger.error(f"Answer failed for workspace {workspace_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def execute_action(
    ...
    if action_label == "Archive":
        ...
        email_record = db.query(TriagedEmail).filter(
            (TriagedEmail.id == item_id) | (TriagedEmail.external_message_id == item_id),
        ...
        token = await get_valid_token(db, workspace_id, provider)
        if not token:
        ...
        try:
            if provider == "google":
                ...
                await client.archive_message(email_record.external_message_id or item_id)
            ...
            logger.error(f"Archive failed for {item_id}: {e}", exc_info=True)
...
@router.get("/history")
@cached_response(ttl_seconds=30, prefix="assist_history")
...
async def compose_email(
    ...
    # Try google first
    token_google = await get_valid_token(db, workspace_id, "google")
    token_outlook = await get_valid_token(db, workspace_id, "outlook")
    ...
    try:
        ...
        if workspace:
             ...
             allowed = final_action_gate(
                 ...
                 draft={
                     ...
                     "body": payload.body,
                     "provider": "google" if token_google else "outlook"
                 },
                 settings=settings,
                 is_explicit_approval=True # Manual action from UI
             )
             
             if not allowed:
                 raise HTTPException(
                     status_code=403, 
                     detail="Action blocked by safety gate. please check content or configuration."
                 )
        ...
        logger.error(f"Compose failed for workspace {workspace_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
